...
*Gaps 16–27 were identified by reviewing the open source codebases and field deployment records of FAO OpenTenure, Cadasta Foundation, and related LADM/STDM literature.*

**16. Competing claims have no first-class representation.** TerraCommons has no mechanism for a neighbour to assert "I claim this land too." Without it, boundary disputes between community members cannot enter the system — they remain off-chain social conflicts while the DHT record shows false certainty. TerraCommons needs to decide whether a parcel is lockable to a single pending claim, or whether overlapping competing claims are admissible and resolved through governance arbitration.

*Partial solution (FAO OpenTenure):* A dispute is structurally identical to a claim — a second claim with overlapping geometry submitted by the disputing neighbour through the standard submission flow, before the community-wide challenge closing date. The moderation committee then adjudicates between two co-existing overlapping claims. No special dispute machinery is required; the standard validation workflow handles both. This is simple and proven in field deployments.

**17. Degenerate GPS geometries are a routine field problem, not an edge case.** FAO OpenTenure rolled back geometry validation constraints twice after field data consistently produced self-intersecting polygons, co-linear vertices, and wrong winding orders from consumer GPS devices. Gap 9 discusses minimum parcel size; this is a different problem occurring at all scales. TerraCommons must decide at submission time whether to reject degenerate geometries or silently repair them. A repair policy (what repairs are permitted, who is informed, what the audit trail is) must be specified before deployment.

*Partial solution (FAO OpenTenure):* Remove hard geometry validation at submission time; accept and store whatever the GPS device produced. Downstream display and overlap detection flags visual problems for human review. This is operationally the only approach that worked at scale in field deployments — rejection proved unsustainable because degenerate geometries arrived constantly and could not be corrected in the field without re-surveying. The tradeoff is that the stored geometry may not accurately represent the claimed boundary; this must be disclosed to validators.

**18. Parcel split and merge operations break an active validation round.** TerraCommons has no equivalent of a parcel lifecycle. If a household submits a claim and then subdivides the parcel before the validation round completes — common for inheritance or partial sale — validators have committed to a parcel that no longer exists as submitted. The protocol needs a defined answer: does the session abort, does it validate the original parcel only, or does it follow the subdivision? The Holochain source chain records action hashes immutably but has no native "this entry supersedes that entry" chain across entries authored by different agents.

*Partial solution (FAO OpenTenure):* A `historic` claim status marks a parcel as terminated by a split or merge event. `terminate_transaction` and `create_transaction` fields on the claim record link the old claim to the new claims it produced, forming a linked chain of claim generations. If a split happened mid-session OpenTenure required manual resolution — there was no automated protocol handling. TerraCommons would need to decide whether to abort an active session on split (forcing re-submission of the child parcels) or to complete the session against the original parcel before allowing subdivision.

**19. Secondary use rights are not recorded and may be silently erased.** TerraCommons records who controls a parcel — the primary titleholder as the community validates it. In customary tenure systems, other household members hold secondary rights inside that parcel: women's kitchen gardens, water access routes, specific crop fields, seasonal grazing corridors. When only the primary right is recorded, secondary rights become legally invisible. This is documented as one of the most common field failures in land documentation programmes globally: women's customary secondary rights were formally eliminated by early single-name titling in multiple African jurisdictions. TerraCommons needs at minimum a mechanism to record secondary rights against a certified claim, and at most a distinct entry type for secondary rights with its own validation workflow.

*Partial solution (STDM — Social Tenure Domain Model, FAO/UN-Habitat):* STDM is an international data standard specifically designed to separate *party* (who holds the right), *right type* (what kind of right — ownership, use, seasonal access, water rights), and *spatial unit* (where) as three independent objects linked by a relationship record. A single parcel can carry multiple parties with different right types simultaneously. Neither OpenTenure nor Cadasta fully implemented this in their production systems; most deployments recorded only the primary right. STDM remains the best available conceptual model for secondary rights and is worth adopting as TerraCommons' data model for the `SecondaryRight` entry type when that is designed.

**20. Accurate documentation can function as a targeting list for eviction.** Cadasta's Lagos, Nigeria deployment explicitly flagged that precise boundary data "could be a shopping list" for authorities targeting informal settlements for demolition. Gap 14 frames privacy as GDPR compliance. The field threat is different: the data is accurate, the protocol worked correctly, and the outcome is harm — the community's own documentation is used against it. TerraCommons needs to consider whether a community can choose to document claims that are visible only within the DHT network and not exportable to the HTTP Gateway, and what the protocol does if a government demands access to DHT node data. This may require a distinct deployment mode (referenced in the Vision & Architecture as "protected-record mode") with different gateway access rules specified from the start of deployment, not applied retroactively.

*Partial solution (Cadasta):* Cadasta's response was to make gateway publication opt-in per community and per claim, with explicit consent required before any data was made externally visible. No data was published publicly until the community had reviewed and approved it. This is a governance solution, not a technical one — it depends on the NGO partner correctly implementing and respecting the consent process. For TerraCommons, the equivalent would be making the HTTP Gateway disabled by default and requiring an explicit governance vote (recorded on the Governance DNA) before enabling public record export for a given community.

**21. Privacy architecture requirements are contextually opposite across deployment environments.** Cadasta found that Ugandan communities needed cloud-distributed backup because local officials could destroy local records, while other communities needed restricted access because officials could obtain cloud records. Holochain's DHT is by design distributed across all participating nodes — it sidesteps the cloud/local binary but does not resolve the underlying question: whose data access rights are encoded into the membrane from deployment? A government-hostile deployment and a government-cooperative deployment need different membrane designs. The threat model must be documented and the membrane designed accordingly before first household registration, not as a later modification.

*Partial solution (Cadasta):* Cadasta offered two deployment configurations — self-hosted (community or NGO controls the server) and Cadasta-hosted (data on Cadasta's cloud infrastructure). Communities chose based on their threat model. Neither fully solved the problem: self-hosted data could still be seized physically; cloud-hosted data could still be subpoenaed. For TerraCommons, Holochain's distributed DHT is inherently closer to the "no single seizure point" model, but the gateway node and the conductor node are still physical devices that can be seized. The membrane design and the gateway access policy are the levers available — these must be documented and chosen per deployment context rather than defaulting to a single configuration.

**22. A validated TerraCommons claim may directly contradict a state-issued title with no record of the conflict.** Cadasta's Zambia deployment found hundreds of hectares of pre-colonial farms where community occupation for decades ran parallel to statutory state titles for the same land. A TerraCommons HarmonyRecord stating that 7 of 9 validators confirmed a claim carries no information about whether a competing statutory title exists, under what legal framework the claim is being made, or which system would take precedence in a legal dispute. Any government or NGO using TerraCommons records as evidence needs to know this. The CertifiedClaim entry should include at minimum a `legal_framework` field (customary, statutory, contested) and a mechanism for recording the existence of known competing claims in other systems.

*Partial solution (Cadasta Zambia):* Cadasta added a `tenure_type` field to claims (freehold, leasehold, customary, occupancy, etc.) and a `conflicting_title` flag with a free-text field for recording known statutory conflicts. This did not resolve the conflicts — it made them visible. The approach was purely documentary: the Cadasta record acknowledged the conflict existed and what the community's claim was, while explicitly stating it made no determination about which claim was legally superior. TerraCommons should adopt the same approach: `legal_framework` and `known_conflicts` fields on `CertifiedClaim`, with clear documentation that a TerraCommons certification is a community validation finding, not a legal determination of title.

**23. Relative GPS accuracy between neighbouring claims matters more than absolute accuracy.** Cadasta's technical teams concluded that relative accuracy — consistency between adjacent claim boundaries — is more operationally important than absolute accuracy. Two adjacent claims with 3-metre GPS drift that appear non-overlapping may be genuinely overlapping when professionally surveyed. Known gap 9 addresses the minimum parcel size below which GPS polygons are unreliable. This gap is different: at any parcel size, the absence of per-vertex accuracy metadata means the overlap detection system cannot distinguish genuine boundary conflicts from GPS noise. TerraCommons should store GPS accuracy estimates with claim boundaries and define a proximity warning zone rather than a hard no-overlap rule.

*Partial solution (Cadasta):* Cadasta stored a `location_accuracy` value (in metres) per spatial point, recorded by the GPS device at capture time. Overlap detection used this to apply a buffer zone — two claims separated by less than the sum of their stated accuracies were flagged as "potentially overlapping" rather than "confirmed overlapping," and sent to human review rather than automatic conflict escalation. This significantly reduced false-positive dispute flags from GPS noise while preserving genuine conflict detection. TerraCommons should add a `gps_accuracy_metres` field per vertex to the `GpsPolygon` type and apply the same buffer logic in overlap detection.

**24. The community-wide challenge window is an undocumented design decision.** FAO OpenTenure uses a single global challenge closing date — all claims in a community close for dispute simultaneously, creating a single community deliberation event. TerraCommons uses per-session timeouts (gap 7). These are different designs with different properties: per-session timeouts allow gaming (a claimant submits early; a neighbour's challenge window expires while they are away from the community); community-wide closing dates create a logistical bottleneck but enforce collective deliberation. TerraCommons has not documented which model it uses or why. This decision should be made explicitly before deployment, in consultation with the community governance design.

*Partial solution (FAO OpenTenure):* A single `moderation_date` system setting — one global closing date for all pending claims in the community, with a fallback `moderation_days` setting when no explicit date is set. All claims submitted before the closing date enter the challenge window simultaneously; claims submitted after it open the next window. This enforces a community-wide deliberation rhythm (monthly or seasonal registration events) rather than a rolling per-claim window. The known failure mode: a community member away during the entire window loses their challenge right permanently. OpenTenure had no mitigation for this — it was documented as a known limitation requiring governance policy (the land committee could accept late challenges at its discretion).

**25. Geographic scoping of claims to an administrative unit must be explicit from deployment.** FAO OpenTenure retrofitted administrative boundary assignment two years after initial deployment; all prior records fell into a `'default-project'` placeholder requiring manual remediation. If TerraCommons defines a "community" by membrane credential alone — who holds the right key — a claimant could submit claims into a neighbouring village's network if their credential allows. Geographic scoping (which parcels belong to which community's jurisdiction) must be enforced at the DNA level from the start, not added after claims exist. The network seed prevents cross-community DHT merging, but it does not prevent cross-geographic claim submission within a single community's network.

*Partial solution (FAO OpenTenure):* After the retrofit, OpenTenure added an `administrative_boundary` table with a polygon defining the geographic extent of the community's jurisdiction, and a `boundary_id` foreign key on every claim. Validation rules then rejected claims whose GPS centroid fell outside the community's boundary polygon. For TerraCommons, the equivalent is including the community's jurisdiction polygon in DNA properties (baked into the DNA hash at deployment) and validating at submission that the claimed parcel falls within it. This is a pre-deployment design requirement: the jurisdiction boundary must be agreed and encoded before the first household registration, not added retrospectively.

**26. Bitemporality is required for historical land rights claims.** Cadasta built `effective_time` and `assert_time` onto every model — when a fact was true in the world versus when the system recorded it. Holochain's source chain records when a commit or reveal was submitted (assert time). Land rights evidence is temporally deep: a household claiming occupation since 1950 based on elder testimony has an effective date of 1950 and an assert date of 2026. These are distinct and both legally significant. TerraCommons currently has no mechanism to record the temporal extent of the right being claimed — only the timestamp of submission. Inheritance disputes (gap 12) and historical occupation claims cannot be properly modelled without at minimum an `effective_from` field on claim submissions, and without a mechanism to attach dated evidence to specific time periods in the occupation history.

*Partial solution (Cadasta data-model-sketches, archived 2019):* Cadasta implemented a `@bitemporal` Python decorator that added `effective_start`, `effective_end`, `assertion_start`, and `assertion_end` to every model, along with `TemporalForeignKey` and `TemporalManyToManyField` relationship types that correctly resolve across time periods. It is unclear whether this made it into Cadasta's production system. LADM (ISO 19152, the international Land Administration Domain Model standard) mandates bitemporality as a formal requirement. For TerraCommons, the minimum viable approach is adding `occupation_start_year` and `occupation_start_confidence` fields to the `ClaimPackage` — allowing a household to assert "we have farmed this since approximately 1970" — and storing dated metadata on individual `EvidenceItem` entries so validators can assess the temporal chain of evidence.

**27. Some communities cannot produce spatial data at all.** FAO OpenTenure introduced a `requires_spatial` system setting (default: mandatory, disableable per deployment) because some field contexts produce no mappable boundary — only oral boundary descriptions, landmark references ("bounded by the river to the north, the old mango tree to the west"), or metes-and-bounds text. These are legitimate land evidence in many customary tenure jurisdictions and accepted in Kenyan land adjudication processes. TerraCommons' commit-reveal protocol presumably commits to a parcel hash that includes spatial data. If spatial data is unavailable — because GPS is non-functional, the terrain is unmappable, or the community uses oral boundary tradition — the current design has no fallback. A text-based boundary description entry type, with its own validation workflow, needs specifying before deployment in contexts where spatial data cannot be guaranteed.

*Partial solution (FAO OpenTenure):* Setting `requires_spatial = 0` allows a claim to be submitted with a free-text boundary description field in place of a GPS polygon. No structured format is enforced; the text is stored as-is and validators read it directly. This is an escape hatch rather than a designed alternative — there is no overlap detection, no adjacency analysis, and no standardised format for the text description. For TerraCommons, the minimum viable approach is a `TextBoundaryDescription` variant in the `GpsPolygon` type that stores the free-text description and marks the claim as non-spatial in the validation record. Validators assessing a non-spatial claim would walk the boundary with the claimant in person rather than reviewing coordinates, and their assessment would note this explicitly. The governance framework would need to document reduced certainty for non-spatial claims.
