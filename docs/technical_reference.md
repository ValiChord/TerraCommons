...
**16. Competing claims have no first-class representation.** TerraCommons has no mechanism for a neighbour to assert "I claim this land too." Without it, boundary disputes between community members cannot enter the system — they remain off-chain social conflicts while the DHT record shows false certainty. FAO OpenTenure resolved this by making a dispute structurally identical to a claim: a second claim with overlapping geometry, submitted by the disputing neighbour before a community-wide challenge closing date. TerraCommons needs to decide whether a parcel is lockable to a single pending claim, or whether overlapping competing claims are admissible and resolved through governance arbitration.

**17. Degenerate GPS geometries are a routine field problem, not an edge case.** FAO OpenTenure rolled back geometry validation constraints twice after field data consistently produced self-intersecting polygons, co-linear vertices, and wrong winding orders from consumer GPS devices. Gap 9 discusses minimum parcel size; this is a different problem occurring at all scales. TerraCommons must decide at submission time whether to reject degenerate geometries or silently repair them. OpenTenure's field experience suggests rejection is operationally unsustainable. A repair policy (what repairs are permitted, who is informed, what the audit trail is) must be specified before deployment.

**18. Parcel split and merge operations break an active validation round.** FAO OpenTenure introduced a `historic` claim status and a termination/transaction chain precisely to handle parcels being subdivided or merged after submission. TerraCommons has no equivalent. If a household submits a claim and then subdivides the parcel before the validation round completes — common for inheritance or partial sale — validators have committed to a parcel that no longer exists as submitted. The protocol needs a defined answer: does the session abort, does it validate the original parcel only, or does it follow the subdivision? The Holochain source chain records action hashes immutably but has no native "this entry supersedes that entry" chain across entries authored by different agents.
